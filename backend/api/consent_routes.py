"""Authenticated, owner-scoped ConsentPulse REST API."""
...
from models.consent import store
from ml.drift_detector import detector
from ml.consent_firewall import firewall
from schemas.consent import (
    ConsentGrantRequest, ConsentUpdateRequest, ConsentWithdrawRequest,
    ConsentSimulateRequest, ConsentReconsentRequest, ProcessingActionRequest,
)
...
def _owned(subject_id: str, user_id: str):
    record = store.get_for_user(subject_id, user_id)
    if not record:
...
async def grant_consent(body: ConsentGrantRequest, user_id: str = Depends(require_user)):
    ...
    if existing and existing.owner_user_id != user_id:
        raise HTTPException(status_code=403, detail="Consent belongs to another user")
    if existing and existing.status == "active":
        return JSONResponse(content=_record_response(existing))
    try:
        record = store.grant(
            subject_id=body.subject_id, owner_user_id=user_id,
            subject_type=body.subject_type, consent_type=body.consent_type,
            purpose=body.purpose, data_categories=body.data_categories,
            collection_scope=body.collection_scope, processing_scope=body.processing_scope,
            duration_days=body.duration_days, version=body.version,
            prohibited_data=body.prohibited_data, allowed_actions=body.allowed_actions,
        )
    except ValueError:
        raise HTTPException(status_code=403, detail="Consent ownership mismatch")
...
async def get_consent(subject_id: str, user_id: str = Depends(require_user)):
    return JSONResponse(content=_record_response(_owned(subject_id, user_id)))


@router.get("/{subject_id}/timeline")
async def get_consent_timeline(subject_id: str, user_id: str = Depends(require_user)):
...
async def update_consent(subject_id: str, body: ConsentUpdateRequest, user_id: str = Depends(require_user)):
    ...
    if body.purpose is not None: cur.purpose = body.purpose
    if body.data_categories is not None: cur.data_categories = body.data_categories
    if body.collection_scope is not None: cur.collection_scope = body.collection_scope
    if body.processing_scope is not None: cur.processing_scope = body.processing_scope
    if body.version is not None: cur.version = body.version
    record.log("processing_scope_changed", "Consent context changed", previous_state=previous,
               new_state=cur.to_dict(), reason=body.reason)
    result = detector.evaluate(record)
    if result.status != "ALIGNED":
        record.log("drift_detected", f"Drift detected: {result.status.replace('_', ' ').title()}",
...
async def reconsent(subject_id: str, body: ConsentReconsentRequest, user_id: str = Depends(require_user)):
    existing = _owned(subject_id, user_id)
    ...
    if (new_purpose, new_data, new_prohibited, new_actions) != (base.purpose, base.data_categories, base.prohibited_data, base.allowed_actions) and new_version == base.version:
        raise HTTPException(status_code=422, detail="Boundary changed but version was not — pass a new `version`")
    record = store.grant(
        subject_id=subject_id, owner_user_id=user_id,
        subject_type=existing.subject_type, consent_type=existing.consent_type,
        purpose=new_purpose, data_categories=new_data,
        ...
        duration_days=body.duration_days or 30, version=new_version,
        prohibited_data=new_prohibited, allowed_actions=new_actions,
...
async def withdraw_consent(subject_id: str, body: ConsentWithdrawRequest, user_id: str = Depends(require_user)):
    ...
    record.log("consent_withdrawn", "Consent withdrawn by subject",
               previous_state={"status": "active"}, new_state={"status": "withdrawn"}, reason=body.reason)
    return JSONResponse(content=_record_response(record))
...
async def simulate_consent(subject_id: str, body: ConsentSimulateRequest, user_id: str = Depends(require_user)):
    ...
    if scenario == "reset":
        record.status = "active"; record.withdrawn_at = None
    ...
    if scenario == "purpose_expansion":
        previous = record.current_context.to_dict()
        if "behavioral_analytics" not in record.current_context.purpose: record.current_context.purpose.append("behavioral_analytics")
        record.log("processing_scope_changed", "Purpose expanded (simulated)", previous_state=previous, new_state=record.current_context.to_dict(), reason="Demo: purpose expansion")
    elif scenario == "data_expansion":
        previous = record.current_context.to_dict()
        if "webcam_analysis" not in record.current_context.data_categories: record.current_context.data_categories.append("webcam_analysis")
        record.log("processing_scope_changed", "Data collection expanded (simulated)", previous_state=previous, new_state=record.current_context.to_dict(), reason="Demo: additional data category")
    elif scenario == "scope_change":
        previous = record.current_context.to_dict(); record.current_context.processing_scope = "cross_session_profiling"
        record.log("processing_scope_changed", "Processing scope changed (simulated)", previous_state=previous, new_state=record.current_context.to_dict(), reason="Demo: processing scope change")
    elif scenario == "expire":
        record.expires_at = time.time() * 1000 - 1000; record.log("consent_expired", "Consent expired (simulated)", reason="Demo: backdated expiry")
        return JSONResponse(content=_record_response(record))
    elif scenario == "withdraw":
        record.status = "withdrawn"; record.withdrawn_at = time.time() * 1000; record.log("consent_withdrawn", "Consent withdrawn (simulated)", reason="Demo: withdrawal")
        return JSONResponse(content=_record_response(record))
    result = detector.evaluate(record)
    if result.status != "ALIGNED": record.log("drift_detected", f"Drift detected: {result.status.replace('_', ' ').title()}", new_state={"status": result.status, "reasons": result.reasons})
    return JSONResponse(content=_record_response(record))
...
async def authorize_action(subject_id: str, body: ProcessingActionRequest, user_id: str = Depends(require_user)):
    ...
    return JSONResponse(content={"subject_id": subject_id, "action_name": body.action_name, **result.to_dict()})
