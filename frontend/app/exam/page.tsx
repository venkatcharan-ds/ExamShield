...
import { QUESTION_BANK, EXAM_NAME, EXAM_DURATION_MINUTES, shuffleQuestions, isAnswerCorrect, type ExamQuestion } from '@/services/questionBank'
...
function grade(questions: ExamQuestion[], answers: Record<string, string>) {
  ...
  for (const q of questions) {
    ...
    if (!value) unanswered++
    else if (isAnswerCorrect(q, value)) correct++
    else incorrect++
  }
...
export default function ExamPage() {
  const [questions] = useState(() => shuffleQuestions(QUESTION_BANK))
  ...
  const submit = useCallback(async () => {
    setResult(grade(questions, answers)); setSubmitted(true); setShowSubmit(false)
